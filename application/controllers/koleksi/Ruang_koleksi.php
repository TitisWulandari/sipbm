...
class Ruang_koleksi extends CI_Controller
{
    ...
    public function index()
    {
        ...
        $data['ruang'] = $this->Ruang_koleksi_model->getAllJoin();
        $data['koleksi'] = $this->Koleksi_model->getAll();
        $this->load->view('templates/header', $data);
        $this->load->view('templates/sidebar');
        $this->load->view('templates/topbar');
        $this->load->view('admin/koleksi/ruang_koleksi/index', $data);
        $this->load->view('templates/footer');
    }

    public function insert()
    {
        $this->form_validation->set_rules('nama_ruang_koleksi', 'Nama Ruang Koleksi', 'required');
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $data = [
                'nama_ruang_koleksi' => $nama_ruang_koleksi,
                'id_users' => $id_users,
                'time_create_ruang_koleksi' => date('Y-m-d H:i:s')
            ];

            $insert = $this->Ruang_koleksi_model->insert($data);

            if ($insert) {

                $this->session->set_flashdata('success', '<div class="alert alert-success" role="alert">Sukses, Data ruang koleksi berhasil ditambahkan !</div>');
                redirect('koleksi/ruang_koleksi');
            }
        }
    }
    public function update()
    {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $nama_ruang_koleksi = $this->input->post('nama_ruang_koleksi');
            $id_users = $this->input->post('id_users');
            $id = $this->input->post('id_ruang_koleksi');

            date_default_timezone_set("ASIA/JAKARTA");
            $data = [
                'nama_ruang_koleksi' => $nama_ruang_koleksi,
                'id_users' => $id_users,
                'time_update_ruang_koleksi' => date('Y-m-d H:i:s')
            ];

            $update = $this->Ruang_koleksi_model->update($id, $data);
    ...
    public function doTambahBerita()
    {
        ...
        if ($this->form_validation->run() ===  FALSE) {
            ...
            if (!$this->upload->do_upload('gambar_koleksi')) {
                ...
                $this->session->set_flashdata('success', '<div class="alert alert-success" role="alert">Sukses, Data koleksi berhasil ditambahkan !</div>');
    ...
    {
        $data['id_ruang_koleksi'] = $this->Ruang_koleksi_model->delete($id);
